...
if [[ -d "$REPO_DIR/.git" ]]; then
    ...
    # A personal access token used to be wired in here, repo-locally, as the way
    # to push. Retired 2026-08-04 with the token itself. It was kept for a few
    # hours as a fallback for "the key is missing on a fresh machine" — until
    # the obvious was noticed: the token lived on the SAME data disk as the key,
    # so the realistic failure (no data disk) took both at once, and the only
    # case it covered was a key that had gone missing on its own. Two secrets
    # for one job, one of them protecting almost nothing.
    #
    # Also relevant, and checked rather than assumed: the repository is public,
    # so bootstrap's clone never needed a credential either.

    # --- GitHub over SSH ---------------------------------------------------
    # The way in since 2026-08-04; the token above is only the fallback. See
    # hosts/desktop/config.sh for why a key rather than a token.
    if [[ -n "${SSH_GITHUB_KEY:-}" ]]; then
        if [[ -s "$SSH_GITHUB_KEY" ]]; then
            # 0700/0600 are not tidiness: ssh refuses a key that group or world
            # can read, and says so in a message that does not mention modes.
            install -d -m 700 "$HOME/.ssh"
            install -m 600 "$SSH_GITHUB_KEY"      "$HOME/.ssh/github_ed25519"
            [[ -s "$SSH_GITHUB_KEY.pub" ]] && \
                install -m 644 "$SSH_GITHUB_KEY.pub" "$HOME/.ssh/github_ed25519.pub"
            install -m 600 "$REPO_DIR/dotfiles/ssh_config" "$HOME/.ssh/config"
            echo "ssh: github key installed from $SSH_GITHUB_KEY"

            # Host key, verified against the pin in config.sh. An unattended
            # install cannot answer the "are you sure" question, and answering
            # it blindly (StrictHostKeyChecking=no) would give away exactly the
            # protection the key exists for.
            if [[ -n "${GITHUB_HOST_KEY_FP:-}" ]]; then
                scan="$(mktemp)"
                if ssh-keyscan -t ed25519 github.com 2>/dev/null > "$scan" \
                   && [[ -s "$scan" ]] \
                   && [[ "$(ssh-keygen -lf "$scan" | awk '{print $2}')" == "$GITHUB_HOST_KEY_FP" ]]; then
                    touch "$HOME/.ssh/known_hosts"
                    chmod 600 "$HOME/.ssh/known_hosts"
                    grep -q '^github.com ssh-ed25519' "$HOME/.ssh/known_hosts" \
                        || cat "$scan" >> "$HOME/.ssh/known_hosts"
                    echo "ssh: github host key verified against the pin"
                else
                    echo "WARNING: github.com host key did not match $GITHUB_HOST_KEY_FP"
                    echo "         known_hosts NOT written — the first push will ask."
                    echo "         Either GitHub rotated the key (check their docs and"
                    echo "         update config.sh) or something is in the way."
                fi
                rm -f "$scan"
            fi
        else
            echo "WARNING: $SSH_GITHUB_KEY is missing — GitHub falls back to the token."
            echo "         Generate one with:"
            echo "         ssh-keygen -t ed25519 -C $GIT_IDENTITY_NAME -N '' -f $SSH_GITHUB_KEY"
            echo "         and add the .pub half at github.com/settings/ssh/new"
        fi
    fi

    # --- git configuration for EVERY repository on the machine --------------
    # Not only this checkout: the point of the includeIf rules is that a
    # project cloned next year is configured correctly without anyone
    # remembering to do it. The identity file is generated rather than shipped
    # so that config.sh stays the single source of the name and address.
    install -Dm644 "$REPO_DIR/dotfiles/gitconfig" "$HOME/.config/git/config"
    cat > "$HOME/.config/git/identity-github" <<EOF
# Generated by phoinix stage 3 from GIT_IDENTITY_* in hosts/*/config.sh.
# Pulled in by ~/.config/git/config, and ONLY for remotes on ulu's own GitHub
# account — never globally. Edit config.sh, not this file.
[user]
	name = $GIT_IDENTITY_NAME
	email = $GIT_IDENTITY_EMAIL
EOF
    chmod 644 "$HOME/.config/git/identity-github"
    echo "git: ~/.config/git/config installed (identity bound to github.com/$GIT_IDENTITY_NAME)"
...
if [[ -n "${VPN_CONFIG_DIR:-}" ]]; then
    ...
    if [[ -n "${VPN_DNS_STUB:-}" ]] && command -v dnsmasq >/dev/null; then
        ...
        # …and restart it whenever the tunnel is rebuilt. dnsmasq binds its
        # upstream socket while the tunnel is up and does not notice when that
        # tunnel is replaced — measured 2026-08-06: after a reconnect three of
        # four lookups timed out until the service was restarted by hand. Since
        # toggling the tunnel from the applet is the whole point of keeping
        # NetworkManager in charge, leaving that to a human would break the one
        # control this design exists to preserve.
        sed -e "s|@VPN_INTERFACE@|$VPN_INTERFACE|g" \
            "$REPO_DIR/system/NetworkManager/dispatcher.d/50-phoinix-vpn-dns" \
            | sudo tee /etc/NetworkManager/dispatcher.d/50-phoinix-vpn-dns >/dev/null
        sudo chmod 755 /etc/NetworkManager/dispatcher.d/50-phoinix-vpn-dns
        echo "vpn: dispatcher restarts the forwarder when $VPN_INTERFACE changes state"
    else
