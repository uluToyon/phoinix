...
- **Dolphin sidebar ordering** (application round, 2026-07-31). ulu reordered
  the disks in the Places panel; KDE stores that as `<separator>` markers in
  `~/.local/share/user-places.xbel`, each naming its device by UDI (`sda1`,
  `nvme0n1p1`, …) and filesystem UUID. Both are unusable in the repo — device
  names drift, and the root/home UUIDs are regenerated by every install. Needs
  runtime resolution from the disk labels in `config.sh`, same principle as the
  panels and the pointer profile.
  His order: root, home, Games, FilesMusic, Downloads, Video.
